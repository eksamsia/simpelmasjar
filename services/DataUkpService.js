...
function detail(id){
  $.ajax({
    dataType: 'json',
    url: 'data_ukp/ambil-data-by-id/'+id,
    success: function ( data) {
      console.log(data.data[0].imgpath);
      ...
      if(!data.data[0].imgpath){
        $("#file_data_ukp").attr("src","<?php echo base_url() ?>upload_file/news_image/no_image_available.jpeg");
      }
      else
      {
        $("#file_data_ukp").attr("src",data.data[0].imgpath);
      }
