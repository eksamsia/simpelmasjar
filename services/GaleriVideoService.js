...
function reload_table()
{
  ...
  function insert()
  {
    ...
    var url = $("#insert-modal").find("input[name='url']").val();
    
    if(tanggal == "" || judul == "" || url=="")
    {
      ...
      $.ajax({
        ...
        success : function (data, status)
        {
          if(data.status != 'error')
          {
            ...
            $("#insert-modal").find("input[name='url']").val(''); 
  ...
  function detail(id){
    $.ajax({
      dataType: 'json',
      url: 'galeri_video/ambil-data-by-id/'+id,
      success: function ( data) {
        var embed_url = youtube_parser(data.data[0].url)
        $("#title h4").text(data.data[0].judul);
        $("#title #oleh").text('Oleh : '+data.data[0].nama);
        $("#title #tanggal").text(data.data[0].tanggal);
        if(!data.data[0].url){
          $("#url_youtube").attr("src","<?php echo base_url() ?>upload_file/news_image/no_image_available.jpeg");
        }
        else
        {
          $("#url_youtube").attr("src",'https://www.youtube.com/embed/'+embed_url);
        }
  ...
  function update(id){
    $.ajax({
      ...
      success: function ( data) {
        ...
        $("#update-modal").find("input[name='url2']").val(data.data[0].url);
  ...
  function update_data()
  {
    ...
    var url = $("#update-modal").find("input[name='url2']").val();
    ...
    $.ajax({
      ...
      success : function (data, status)
      {
        if(data.status != 'error')
        {
          $(".modal").modal('hide');
          reload_table();
          $("#update-modal").find("input[name='url2']").val('');
          toastr.success(data.msg, 'Success Alert', {timeOut: 5000});
        }
        else{
          toastr.error(data.msg, 'Error Alert', {timeOut: 5000});
          ...
          hapus: {
            ...
            action: function(){
              $.ajax({
                dataType: 'json',
                type:'POST',
                url: 'galeri_video/remove_data/'+id,
                data:{
                  id:id
                }
              }).done(function(data){
